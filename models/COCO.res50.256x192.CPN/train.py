...
def main(args):
    ...
    # create model
    model = network.__dict__[cfg.model](cfg.output_shape, cfg.num_skeleton, pretrained=True)
    model = torch.nn.DataParallel(model).cuda()

    # criterion
    criterion1 = torch.nn.MSELoss().cuda()              # global loss
    criterion2 = torch.nn.MSELoss(reduce=False).cuda()  # refine loss
    ...
    train_loader = torch.utils.data.DataLoader(
        Mscoco(cfg), batch_size=cfg.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
...
def train(train_loader, model, criterions, optimizer):
    ...
    for i, (inputs, targets, valid, meta) in enumerate(train_loader):
        ...
        # compute gradient and optimeze
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        if (i%100 == 0 and i != 0):
            print('iteration {} | loss: {}, global loss: {}, refine loss: {}, average loss: {}'
            .format(i, loss.data.item(), global_loss_record, refine_loss_record, losses.avg))
    return losses.avg
